...
| `APIARY_DERIVED_SHADOW` | `1` | Run the derived-state resolver beside the `swarm:*` labels every cycle and record where the two disagree (#146). Read-only: the labels stay authoritative and nothing in the loop reads the derived value. It adds no API call, and it also records `observed.jsonl` into the run directory, which is what makes a real run replayable as a test corpus. `swarm show` reports the divergence count. |
