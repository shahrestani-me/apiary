...
def test_greenfield_provisions_an_empty_repository_gated_by_the_stacks_own_command(
    monkeypatch,
):
    """This test's premise has now inverted twice, and #293 is the second time.

    It first asserted that `--new` must not build a plain `ProvisionPlan`,
    because the repository that produced had no test suite and the planner then
    wrote issues verified with a runner it could not run. #104 inverted that:
    the project became the first *issue* of the plan, so the initial commit is
    deliberately almost empty and `test -f README.md` was the only command that
    could gate it. What had been the bug became the design.

    What made that safe was one sentence, and the sentence was false: "the
    bootstrap's PR replaces both the files and the gate before any other task
    runs". A worker cannot replace the gate. Rewriting `.github/workflows`
    needs the `workflows` permission and the work key is forbidden it, and
    `BOOTSTRAP_FILES` never declared the workflow in the first place - so the
    placeholder was not a starting condition, it was permanent. Measured on a
    live run: every issue of `fantasy-bestiary-2` demanded comprehensive unit
    tests and carried `## Verify: test -f README.md`, a gate that cannot fail.

    So the plan now carries the stack's real command. The initial commit is
    still three files and still has no project in it - that part of #104 stands,
    and `test_the_initial_commit_is_red_until_the_bootstrap_lands` is the
    consequence this buys and accepts.
    """
    code, provisioning, _ = greenfield(monkeypatch, FakeClient([]))

    assert code == 0
    assert provisioning.plan.verify_command == STACK_VERIFY["python"]
    assert provisioning.plan.verify_command != PLACEHOLDER_VERIFY
    # Three files, and no project: the README, the LICENSE, and the workflow
    # that runs the gate.
...
def test_the_initial_commit_is_red_until_the_bootstrap_lands(monkeypatch, tmp_path):
    """The gate is real, so it fails on a repository with no code. Deliberately.

    This test used to assert the opposite - that the initial commit *passes* the
    command its issues carry - and satisfying it was what pinned every
    greenfield repository to `test -f README.md` forever. The two properties
    cannot both hold: a command that passes on a commit containing a README, a
    LICENSE and a workflow is a command that grades an empty generation green,
    which is exactly what #102's falsification exists to forbid.

    Given the choice, a red badge on the initial commit is worth far more than
    an unfailable gate, and it costs nothing that matters: the ruleset requires
    the check on *pull requests*, and a PR is gated on its own run, not on the
    state of the branch it targets (`rules()` keeps
    `strict_required_status_checks_policy` False for an unrelated reason, and it
    helps here too). So the first PR - the bootstrap's, which writes the test
    file - is the commit that turns the badge green, and nothing is blocked in
    the meantime.

    Run through a `python3` that is this interpreter, because the assertion is
    about *collecting no tests* and a host whose `python3` has no pytest would
    produce the same non-zero exit for an unrelated reason and pass this test
    for the wrong cause.
    """
    code, provisioning, planning = greenfield(monkeypatch, FakeClient([]))
    ...
    # An exec shim rather than a symlink: a virtualenv interpreter finds its
    # `pyvenv.cfg` from its own path, so a symlink parked outside the venv
    # resolves to the base interpreter's site-packages and loses pytest - which
    # is the one failure this test must not be able to confuse for its subject.
    shim = tmp_path / "bin"
    shim.mkdir()
    ...
    assert code == 0
    # 5 is pytest's "no tests collected", not an error: the gate ran, found
    # nothing to run, and refused to call that a pass.
    assert result.returncode == 5, result.stdout + result.stderr
    assert "no tests ran" in result.stdout.lower()
