...
| `results` | the result files this cycle was handed | the **name on disk** of each record the cycle read, carried from the read that produced them (`Reconciler._results`) rather than rebuilt from the record's `attempt` — since #177 several records can share an attempt under different names, and only the name tells them apart; a name that is not under `results/` fails the load, and the visible set is reduced to the newest record per issue (`worker.result.latest_named`, which is `summarise_dir(...).latest` with the file attached), so a task carries one `AttemptFact` on both sides of the recording |
